"""Fine-tune: recovery cap, tox center terms, and pHat alpha."""
import subprocess
import re
...
contract Strategy is AMMStrategyBase {{
    ...
    {{
        ...
        {{
            ...
            uint256 pAlpha = firstInStep ? WAD * {phat_alpha} / 100 : WAD * 5 / 100;
            if (ret <= adaptiveGate) {{
...
base = dict(rec_cap=1000, tox_center="", phat_alpha=26)

# 1. Recovery cap sweep
print("=== Recovery cap sweep ===")
for cap in [500, 800, 1000, 1500, 2000, 3000, 5000]:
    p = {**base, 'rec_cap': cap}
    e = run_test(p)
    print(f"  rec_cap={cap}: {e}")
...
tox_terms = [
    ...
    ("100*tox+5000*tox2", "        center = center + wmul(bpsToWad(100), tox) + wmul(bpsToWad(5000), wmul(tox, tox));"),
    ("200*tox+8000*tox2", "        center = center + wmul(bpsToWad(200), tox) + wmul(bpsToWad(8000), wmul(tox, tox));"),
]
for label, code in tox_terms:
    p = {**base, 'tox_center': code}
    e = run_test(p)
    print(f"  {label}: {e}")

# 3. pHat alpha sweep (how fast pHat tracks the true price)
print("\n=== pHat alpha sweep ===")
for alpha in [15, 20, 26, 32, 40, 50]:
    p = {**base, 'phat_alpha': alpha}
    e = run_test(p)
    print(f"  phat_alpha={alpha}%: {e}")
