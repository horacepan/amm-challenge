"""Last squeeze around the 486.76 optimum."""
...
def test(name, **overrides):
    defaults = {
        ...
        'vol_alpha': 40, 'vol_boost': 450,
        'base_fee': 16, 'linear_coef': 4000, 'quad_coef': 8000,
    }
...
if __name__ == "__main__":
    ...
    print("=== Fine grid around optimum ===")
    for vb in [425, 440, 450, 460, 475, 500]:
        for lc in [3500, 3800, 4000, 4200, 4500]:
            for qc in [5000, 6000, 7000, 8000, 9000, 10000]:
                s, p = test(f"vb{vb}+lin{lc}+q{qc}", vol_boost=vb, linear_coef=lc, quad_coef=qc)
                if s > best: best, best_p = s, p

    print(f"\nBest grid: {best:.2f}")
    bvb = best_p.get('vol_boost', 450)
    blc = best_p.get('linear_coef', 4000)
    bqc = best_p.get('quad_coef', 8000)
    print(f"vb={bvb}, lin={blc}, q={bqc}")

    # Fine tune remaining
    for bf in [14, 15, 16, 17, 18]:
        s, p = test(f"bf{bf}", vol_boost=bvb, linear_coef=blc, quad_coef=bqc, base_fee=bf)
        if s > best: best, best_p = s, p
    bbf = best_p.get('base_fee', 16)
